        ...
        const fetchUsers = async () => {
            try {
                const response = await axios.get(
                    `http://${config.baseURL}/users/users/all`
                );
    ...
    const handleDelete = async () => {
        try {
            const response = await axios.delete(
                `http://${config.baseURL}/users/users/?user_id=${selectedUser?.userID}`
            );
            if (response.status === 200) {
                setUsers((prevUsers) =>
                    prevUsers.filter(
    ...
    const handleUpdate = async () => {
        try {
            ...
            const response = await axios.put(
                `http://${config.baseURL}/users/users/${selectedUser.userID}/update/`,
                updatedUser
            );
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal}>
                ...
                <ModalContent>
                    ...
                    <ModalBody>
                        ...
                        />
                        <Text mb="8px">
                            Current Email: {selectedUser?.emailAddress}
                        </Text>
                        <Input
                            name="emailAddress"
